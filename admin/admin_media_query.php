<?php
define( 'QRCODES_MEDIA_QUERY_OPTION_NAME', 'qrcodes-media-query-network' );
...
function qrcodes_media_query_admin_network_menu() {
	...
	add_action( "load-{$slug}", 'qrcodes_media_query_admin_network_process_remove' );
	add_action( "load-{$slug}", 'qrcodes_media_query_admin_network_load_media_query', 12 );
}
if ( is_multisite() ) {
	add_action( 'network_admin_menu', 'qrcodes_media_query_admin_network_menu' );
} else {
	add_action( 'admin_menu', 'qrcodes_media_query_admin_network_menu' );
}
...
function qrcodes__media_query_admin_network_notices() {
    settings_errors( 'media-query' );
    settings_errors( 'qrcodes-media-query' );
    settings_errors( 'qrcodes-media-query-network' );
}
...
function qrcodes_media_query_admin_script( $page ) {
	if ( 'settings_page_qrcodes-media-query-network-page' == $page ) {
		wp_enqueue_script(
			'qrcodes-admin',
			plugins_url( path_join( '/script', 'admin_network.js') , QRCODES_INDEX_FILE ),
			array( 'jquery' ),
			'0.1',
			true
		);
...
function qrcodes_media_query_admin_current_screen() {
	global $media_list;

	require_once path_join( __DIR__, 'media_query_list_table.php' );
	...
	$media_list = new Qrcodes_Media_Query_List_Table( array(
		...
		'option_group' => 'qrcodes-media-query-network-list-group',
	) );
...
add_action( 'admin_init', 'qrcodes_media_query_admin_current_screen' );
